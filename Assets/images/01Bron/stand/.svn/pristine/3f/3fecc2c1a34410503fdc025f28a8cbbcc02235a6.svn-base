...
public class AstarPath : VersionedMonoBehaviour {
	...
	/** Holds all graph data *//** 保存所有图形数据 */
	[UnityEngine.Serialization.FormerlySerializedAs("astarData")]
	public AstarData data;

    /** Holds all graph data.保存所有图形数据
	 * \deprecated The 'astarData' field has been renamed to 'data'弃用“astarData”字段已被重命名为“data”
	 */
    [System.Obsolete("The 'astarData' field has been renamed to 'data'")]
	public AstarData astarData { get { return data; } }

    /** Returns the active AstarPath object in the scene.返回场景中活动的AstarPath对象。
	 * \note This is only set if the AstarPath object has been initialized (which happens in Awake).注意这仅在AstarPath对象已被初始化（发生在唤醒中）的情况下设置。
	 */
    ...
    //Pathfinding.AstarData.graphs的快捷方式
    ...
    * 注意仅在编辑器中有关
     ...
     * 如果设置，则#debugFloor和#debugRoof值将不会自动重新计算。
	 *
	 * \note Only relevant in the editor
	 */
    ...
    * todo添加一个showOnlyLastPath标志来指示是否绘制每个节点或只绘制最近路径访问的节点。
	 */
    ...
    /** Called on Awake before anything else is done.在任何事情完成之前，唤醒唤醒。
    ...
    * 在唤醒调用开始时，在#active被设置之后调用这个函数，但是这是唯一已经完成的操作。\ n
    ...
    * 当您想要设置运行期间创建的AstarPath组件的默认设置时使用此选项，因为某些设置只能在唤醒中更改
	 * (such as multithreading related stuff)（如多线程相关的东西）
	 * \code
	 * //Create a new AstarPath object on Start and apply some default settings在开始时创建一个新的AstarPath对象并应用一些默认设置
	 * public void Start () {
    ...
    /** 在搜索之前为每个路径调用。 使用多线程时要小心，因为这将从另一个线程中调用。*/
    ...
    /** 搜索后为每个路径调用。 使用多线程时要小心，因为这将从另一个线程中调用。*/
    ...
    * 注意每次调用时都会清除该回调，因此如果您想重复注册，则在收到回调时也可以直接注册回调。
	 */
    ...
    /** Time the last graph update was done.上次图形更新完成的时间。
...
#line default
     ...
     * 如果限制图形更新，此功能非常有用，但您希望尽快应用特定的图形更新，而不受时间限制的限制。
	 * \see FlushGraphUpdates请参阅FlushGraphUpdates刷新图形更新
	 */
    ...
    /** Forces graph updates to complete in a single frame.强制图形更新在单个框架中完成。
     ...
     * 这将强制寻路线程完成计算他们当前正在计算的路径（如果有的话），然后暂停。
     ...
     * \警告经常使用（每秒多次）可以减少你的fps由于大量的线程在等待另一个。
